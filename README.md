...
2-2.안드로이드 DB 연동 이슈 : 라즈베리 파이에는 정상적으로 DB를 올렸지만 안드로이드 앱에서는 라즈베리 파이의 DB에 데이터를 업로드 할 수 없었습니다. 안드로이드는 보안 상의 문제로 웹으로 변환 후 JSON으로 넘겨야 함을 알게 되었고, 이를 해결하기 위해 *DB 연동 문제 해결 내용 작성 후 업로드* 
   + 문제 해결 이전 DB 상황
