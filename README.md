...
##### 2. 진행 중 이슈
2-1. 라즈베리 파이와 깃 허브 연동 : 팀 프로젝트의 진행사항을 저장하고 정리하기 위해 깃 허브 레퍼지토리에 업로드를 진행하려고 했으나 라즈베리와 깃 허브가 연동되지 않아서 commit 된 파일이 업로드 되지 않았습니다. 이것을 해결하기 위해 자료를 찾아보던 중 깃 허브의 정책 변경으로 *토큰*이 필요하다는 것을 알게되었고, 레파지토리가 만들어진 팀원의 깃허브 계정 토큰을 생성 후 이를 입력해 업로드를 시도했더니 성공적으로 업로드 되었습니다.</br>
</br>
2-2.안드로이드 DB 연동 이슈 : 라즈베리 파이에는 정상적으로 DB를 올렸지만 안드로이드 앱에서는 라즈베리 파이의 DB에 데이터를 업로드 할 수 없었습니다. 안드로이드는 보안 상의 문제로 웹으로 변환 후 JSON으로 넘겨야 함을 알게 되었고, 이를 해결하기 위해 *DB 연동 문제 해결 내용 작성 후 업로드* </br>
...
2-3. 아두이노 값 밀림 이슈 : 아두이노 자동시 입력 받은 값이 1개씩 밀려서 저장되는 이슈가 발생했습니다. *문제 화면 화면과 해결 화면을 같이 올리고 어떻게 해결했는지 간단하게 기술*
...
##### 2. 프로젝트를 마치며...
- 프로젝트 총평 (목표와 비교하여 얼만큼 진행 되었는지, 완성도에 대한 내용)
- 아쉬운 점
- 배운 점
